...
struct Node {
    ...
    Node(const string& kode, const string& tujuan, const string& status)
        : kodePenerbangan(kode), tujuan(tujuan), status(status), prev(nullptr), next(nullptr) {}
};
...
void printHeader() {
    ...
    cout << "|           [ KELVIN ALVIANSYAH - NIM ..." << NIM_LAST3 << " ]                   |\n";
    cout << "+------------------------------------------------------------+\n";
}
...
void printMenu() {
    ...
    cout << "| 7. Cari Jadwal (berdasarkan kode atau tujuan)              |\n";
    cout << "| 0. Keluar                                                  |\n";
    cout << "+------------------------------------------------------------+\n";
    cout << "Pilih menu: ";
}
...
void insertAtPosition(List& list, Node* node, size_t pos) {
    ...
    Node* cur = list.head;
    for (size_t i = 1; i < pos - 1 && cur->next; ++i) {
        cur = cur->next;
    }
    node->next = cur->next;
    node->prev = cur;
...
Node* findByCode(List& list, const string& kode) {
    Node* cur = list.head;
    while (cur) {
        if (cur->kodePenerbangan == kode) return cur;
        cur = cur->next;
    }
    return nullptr;
}

Node* findByTujuan(List& list, const string& tujuan) {
    Node* cur = list.head;
    while (cur) {
        if (cur->tujuan == tujuan) return cur;
        cur = cur->next;
    }
    return nullptr;
}
...
void showAllReverse(const List& list) {
    cout << "\nDaftar Semua Jadwal dari Belakang (" << list.size << "):\n";
    if (!list.tail) {
        cout << "(Kosong)\n";
        return;
    }
    const Node* cur = list.tail;
    size_t i = list.size;
    while (cur) {
        cout << i << ". Kode: " << cur->kodePenerbangan
             << " | Tujuan: " << cur->tujuan
    ...
    while (running) {
        ...
        switch (choice) {
            case 1: {
                ...
                string status = readLineSafe("Masukkan status awal: ");
            ...
            case 7: {
                string keyword = readLineSafe("Masukkan kode atau tujuan yang ingin dicari: ");
                Node* target = findByCode(list, keyword);
                if (!target) target = findByTujuan(list, keyword);
                if (!target) {
                    cout << "Data tidak ditemukan.\n";
                } else {
                    cout << "Detail ditemukan:\n";
                    cout << "Kode   : " << target->kodePenerbangan << "\n";
                    cout << "Tujuan : " << target->tujuan << "\n";
                    cout << "Status : " << target->status << "\n";
                }
                pauseAndClear();
                break;
            }
